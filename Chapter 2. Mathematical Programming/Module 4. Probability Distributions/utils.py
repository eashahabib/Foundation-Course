...
#7:
def visualize_normal_interval(interval_floor, interval_ceil, pdf_func, bw, mean, std):
	# Determined
	x_true = np.linspace(mean - std*4, mean + std*4, 1000)
	f_true = normal(x_true, mean, std)

	fig = go.Figure()
	fig = add_hist_approx(fig, interval_floor, interval_ceil, pdf_func, bw)
	fig.add_trace(go.Scatter(x=x_true, y=f_true, marker_color="black"))
	fig.update_layout(
